...
export class GitHubTreeView extends ItemView {
    ...
    async onOpen() {
        ...
        if (repo?.owner && repo?.name) await this.loadTree(repo);
    }
    ...
    private render() {
        ...
        if (this.plugin.settings.repositories.length === 0) {
            ...
            content.createDiv({
                ...
                text: "Select a repository to browse.",
            });
        }
    }
    ...
    private renderHeader(header: HTMLElement) {
        ...
        if (this.plugin.settings.repositories.length > 0) {
            ...
            for (const repo of this.plugin.settings.repositories) {
                const label = repo.displayName || `${repo.owner}/${repo.name}`;
        ...
        iconBtn(actions, "refresh-cw", "Refresh", async () => {
            const repo = this.getActiveRepo();
            if (repo) { this.api.clearCache(repo); await this.loadTree(repo); }
        });
    ...
    // Re-render only the content area (no full re-render)
    ...
    private matchesSearch(node: TreeNode): boolean {
        ...
        if (node.type === "folder") {
            return node.children.some((c) => this.matchesSearch(c));
        }
        return false;
    }
    ...
    private toggleFolder(
        ...
        if (isOpen) {
            ...
            // Render children, measure, then animate
            childrenEl.empty();
            childrenEl.style.height = "0px";
            this.renderNodes(childrenEl, node.children, depth + 1);
    ...
    private renderFile(container: HTMLElement, node: TreeNode, depth: number) {
        ...
        // Spacer aligns with folder chevron
        ...
        row.addEventListener("click", () => {
            const repo = this.getActiveRepo();
            if (repo) window.open(this.api.getFileUrl(repo, node.path), "_blank");
        });
        ...
        row.addEventListener("contextmenu", (e) => {
            ...
            menu.addItem((i) =>
                i.setTitle("Copy Raw URL").setIcon("link-2").onClick(() => {
                    navigator.clipboard.writeText(this.api.getRawUrl(repo, node.path));
                    new Notice("Raw URL copied!");
                })
            );
            menu.showAtMouseEvent(e);
        });
